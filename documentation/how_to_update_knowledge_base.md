...
**`batch_size`**: `int`
> 
> Note that if `batch_size` is too high then you will get error messages back from OpenAI (see **Knownn issues**).


## Known issues

throttle doesn't work, and was also getting error messages back from OpenAI when we couldn't have been exceeding the rate limits.
